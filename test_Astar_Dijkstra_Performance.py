...
class TestSetting(unittest.TestCase):
    ...
    # heuristic function of A* in part 5 based on haversine_distance
...
class CompareDijAndA(TestSetting):
    def test_performance_compare(self):                                              # whole instance of class CompareDijAndA
        ...
        for (source, destination) in generated_random_pairs:
            ...
            dijkstra_times.append(end_Dijkstra - start_Dijkstra)
            ...
            A_star_times.append(end_Astar - start_Astar)
            ...
            for i in range(len(shortest_path_A) - 1):                               # iterate to len(LIST) - 1 since we need to access the last element of the list by [i + 1] 
                ...
                dist_A += self.haversine_distance(self.stations[u], self.stations[v])   # directly call haversine_distance() instead of lambda
...
def draw_plot(dijkstra_times, a_star_times, station_pair_num):
    ...
    save_path = os.path.join(results_dir, "A_star_vs_Dijkstra.png")
...
class PerformanceLineUsage(TestSetting):
    ...
    def test_same_line(self):
        ...
        _, shortest_path = A_star(                                                  # only need shortest path, no need the predecessor dictionary (came_from)
            ...
            lambda u, v: self.heuristic(u, v, self.stations)
        )
        lines = self.compute_lines_used(shortest_path, self.edge_to_line)
        print(f"Same line, Path from {source} to {destination}: {shortest_path}")
    ...
    def test_performance_by_scenario(self):
        scenarios = [ ("Same Line", self.same_line_pair),
            ("Adjacent Lines", self.adjacent_line_pair),
            ("Several Transfers", self.several_transfers_pair)]
        
        scenario_labels = []
        dijkstra_times = []
        astar_times = []

        for single_scenario, (source, destination) in scenarios:
            ...
            end_Astar = time.perf_counter()
            current_astar_time = end_Astar - start_Astar

            # Dijkstra 
            start_Dijkstra = time.perf_counter()
            _, shortest_path = dijkstra(self.graph, source, (self.graph.get_num_nodes() - 2))       
            end_Dijkstra = time.perf_counter()
            current_dij_time = end_Dijkstra - start_Dijkstra


            scenario_labels.append(single_scenario)
            astar_times.append(current_astar_time)
            dijkstra_times.append(current_dij_time)

            print(f"{single_scenario} Scenario - Source: {source}, Destination: {destination}")
            print(f"A* time: {current_astar_time:.6f} s, Dijkstra time: {current_dij_time:.6f} s")

        #  when loop finish, use draw plot function to illustrate all figures
        draw_plot_scenarios(scenario_labels, dijkstra_times, astar_times)


def draw_plot_scenarios(labels, dijkstra_times, astar_times):
    results_dir = "experiment_results"
    os.makedirs(results_dir, exist_ok=True)
    ...
    plt.bar([xi - width/2 for xi in x], dijkstra_times, width, label="Dijkstra")
    plt.bar([xi + width/2 for xi in x], astar_times, width, label="A*")   
    plt.xticks(x, labels)
    plt.xlabel("Scenario")
    plt.ylabel("Time (seconds)")
    plt.title(f"Performance Comparison by Scenario")
    plt.legend()

    save_path = os.path.join(results_dir, "performance_comparison_by_scenario.png")
    plt.savefig(save_path)
    print(f"Plot saved to {save_path}")
    plt.close()        


# execute file if run directly    
